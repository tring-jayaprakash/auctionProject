...
const MyAuction = () => {
    ...
    async function updateTeamBudget(element, finalBudget) {
        const query = `
        mutation{
          updateTeamBudget(auction_id:${element.auction_id},budget:${finalBudget})
        }
        `
        try {
            const response = await axios.post("http://localhost:2500/graphql", { query });
    ...
    const handleStartAction = async (element, index) => {
        try {
            ...
            if (teamData.length * minPlayer <= playerData.length) {
                console.log(teamData.length * minPlayer >= playerData.length);
                                ...
                                return (
                                    <div className='auction-div' key={index}>
                                        ...
                                        <div className='auction-div-footer'>
                                            ...
                                            <div>
                                                <p><AiFillEdit size={25} title='edit' style={{ cursor: "pointer", color: "#008000" }} onClick={() => { handleEdit(index, element) }} /></p>
                                                <p><MdDeleteForever size={25} title='delete' style={{ cursor: "pointer", color: "#FF0000" }} onClick={() => { handleDelete(index, element.auction_id) }} /></p>
                                            </div>
                                        </div>
                                    </div>
                                );
